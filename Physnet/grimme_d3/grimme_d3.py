...
"""
Tensorflow implementation of Grimme's D3 method (only Becke-Johnson damping is 
implemented) 
...
def _smootherstep(r, cutoff):
    ...
    x4 = x3*x
    x5 = x4*x
    return tf.where(r <= cuton, 
                    tf.ones_like(x), 
                    tf.where(r >= cutoff, tf.zeros_like(x), 6*x5-15*x4+10*x3))
...
def edisp(Z, r, idx_i, idx_j, cutoff=None, r2=None, r6=None, r8=None, s6=d3_s6,
    ...
    nc = _ncoord(Zi, Zj, r, idx_i, cutoff=cutoff, rcov=rcov) 
