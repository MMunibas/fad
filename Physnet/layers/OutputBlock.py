...
class OutputBlock(NeuronLayer):
    ...
    def __init__(self, F, num_residual, activation_fn=None, seed=None, 
                 rate=0.0, dtype=tf.float32, scope='', **kwargs):
        
        super(OutputBlock, self).__init__(F, 2, activation_fn)
        ...
        with tf.name_scope(scope):
            ...
            self._dense = DenseLayer(
                F, 2, W_init=tf.zeros([F, 2], dtype=dtype), use_bias=False,
                scope="dense_layer", dtype=dtype)
